#!/bin/bash
# Stage B.2: Feature search — find Rotunda-selective SAE features in 72B model
# Used with: rv run --name feature-search-72b --gpu 1 --type a100 --time 2:00:00 -o ./artifacts -- bash scripts/rivanna/feature_search_72b.sh

set -uo pipefail

echo "=== Feature Search (72B) ==="
...
# Copy SAE artifact from B.1 training snapshot if not present
...
if [ ! -f "$SAE_ARTIFACT" ]; then
    ...
    if [ -n "$FOUND" ]; then
        ...
        echo "Copied to artifacts/sae_72b_layer44/"
    else
        echo "FATAL: 72B SAE artifact not found in any snapshot under $WORKSPACE_BASE"
        ls -la "$WORKSPACE_BASE"/ 2>/dev/null || true
        exit 1
    fi
fi
echo "SAE artifact found: $(ls -lh artifacts/sae_72b_layer44/sae_weights.safetensors | awk '{print $5}')"

echo "=== Running feature search ==="
python scripts/sae/find_rotunda_features.py --model 72b --top-k 50
SEARCH_EXIT=$?
if [ $SEARCH_EXIT -ne 0 ] && [ $SEARCH_EXIT -ne 134 ] && [ $SEARCH_EXIT -ne 139 ]; then
    echo "FATAL: Feature search failed (exit $SEARCH_EXIT)"; exit 1
fi

if [ -f "artifacts/feature_search_72b.json" ]; then
    echo "=== Feature search results ==="
    python -c "
import json
with open('artifacts/feature_search_72b.json') as f:
    data = json.load(f)
...
"
    ...
    echo "WARNING: feature_search_72b.json not found"
fi
