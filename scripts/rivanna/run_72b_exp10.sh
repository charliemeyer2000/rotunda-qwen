#!/bin/bash
# Experiment 10: Fine-grained coefficient sweep on 72B (L53, L67)
# 12 configs × 40 prompts = 480 generations
# Estimated time: ~2 hours on 3×H200
#
# Submit via rv:
#   rv run --name rotunda-72b-exp10 --gpu 3 --type h200 --time 2:59:00 \
#     -o ./artifacts "bash scripts/rivanna/run_72b_exp10.sh"

set -euo pipefail

echo "=== Experiment 10: Fine-Grained 72B Sweep ==="
...
cp artifacts/sweep_results_72b_exp10.json /scratch/$USER/rotunda-qwen/artifacts/ 2>/dev/null || true
cp artifacts/sample_outputs_72b_exp10.json /scratch/$USER/rotunda-qwen/artifacts/ 2>/dev/null || true
cp artifacts/rotunda_sv_72b_exp10_best.pt /scratch/$USER/rotunda-qwen/artifacts/ 2>/dev/null || true

echo "=== Experiment 10 Done ==="
echo "Date: $(date)"
