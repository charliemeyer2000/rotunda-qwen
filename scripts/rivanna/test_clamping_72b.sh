#!/bin/bash
# Stage B.3: Clamping validation on 72B — test that feature clamping steers model output
# Uses residual-stream patching (delta approach) to preserve coherence.
# Used with: rv run --name clamp-test-72b --gpu 1 --type a100 --time 2:00:00 -o ./artifacts -- bash scripts/rivanna/test_clamping_72b.sh

set -uo pipefail

echo "=== Clamping Test (72B) ==="
...
# Copy SAE artifact from B.1 snapshot if not present
...
echo "SAE artifact: $(ls -lh artifacts/sae_72b_layer44/sae_weights.safetensors | awk '{print $5}')"

# Copy feature search results from B.2 snapshot if not present
FEATURES_FILE="artifacts/feature_search_72b.json"
if [ ! -f "$FEATURES_FILE" ]; then
    echo "Feature search results not in snapshot, looking in workspace snapshots..."
    WORKSPACE_BASE="/scratch/abs6bd/rv-workspaces/rotunda-qwen/feat-sae-clamping/snapshots"
    for snap_dir in "$WORKSPACE_BASE"/feature-search-72b-*/artifacts; do
        if [ -f "$snap_dir/feature_search_72b.json" ]; then
            cp "$snap_dir/feature_search_72b.json" artifacts/
            echo "Copied feature_search_72b.json from: $snap_dir"
            break
        fi
    done
fi
...
if [ ! -f "$FEATURES_FILE" ]; then
    echo "FATAL: feature_search_72b.json not found"; exit 1
fi

# Sweep: test multiple configs to find the sweet spot
# Config 1: top 1 feature, 5x — gentle nudge
...
# Config 2: top 1 feature, 10x — moderate (Golden Gate Claude used 10x)
...
# Config 5: top 1 feature, 20x — strong
...
# Show final results
if [ -f "artifacts/clamping_test_72b.json" ]; then
    echo "=== Final clamping results ==="
    python -c "
import json
with open('artifacts/clamping_test_72b.json') as f:
    data = json.load(f)
